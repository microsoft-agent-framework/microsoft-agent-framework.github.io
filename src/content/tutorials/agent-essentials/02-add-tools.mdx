...
prerequisites:
  ...
  - "Completion of 'Build Your First AI Agent'"
...
envVars:
  ...
  - "OPENAI_MODEL_NAME"
runCommands:
  - "dotnet new console -n IncidentTriage.Tools -f net10.0"
  - "cd IncidentTriage.Tools"
  ...
  - "dotnet run"
expectedOutput:
  - "The agent identifies that it needs to check the 'checkout' service status."
  - "The agent calls the C# method and receives the 'degraded' status."
  - "The agent summarizes the impact and suggests investigation steps based on the tool's data."
next: "/learn/agent-essentials/multi-turn"
---
