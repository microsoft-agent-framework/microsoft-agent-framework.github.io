...
prerequisites:
  ...
  - "Completion of 'Maintain Context with Multi-Turn Conversations'"
...
envVars:
  ...
  - "OPENAI_MODEL_NAME"
runCommands:
  - "dotnet new console -n IncidentTriage.Memory -f net10.0"
  - "cd IncidentTriage.Memory"
  - "dotnet add package Microsoft.Agents.AI.OpenAI"
  - "dotnet add package Azure.AI.OpenAI"
  - "dotnet restore"
  - "dotnet run"
expectedOutput:
  - "The agent remembers the Incident ID across application restarts."
  - "The session is successfully serialized to a JSON representation."
  - "The AIContextProvider extracts and injects the 'Lead Responder' context."
next: "/learn/agent-essentials/host-your-agent"
---
...
<div class="solid-callout solid-callout-info mb-8">
  ...
  <ul class="space-y-2.5 m-0 p-0 list-none text-sm text-indigo-900/80">
    <li class="flex items-center gap-2">🛠️ **.NET 10.0 SDK** installed.</li>
    <li class="flex items-center gap-2">🤖 **AI Provider**: Azure OpenAI or Local (Ollama/LM Studio).</li>
    <li class="flex items-center gap-2">💾 **Persistence**: We will use `SerializeSessionAsync` to demonstrate state saving.</li>
  </ul>
</div>
...
### <span class="step-pill">1</span> Define the Memory Provider <div class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-indigo-50 border border-indigo-100 text-[10px] font-bold text-indigo-600 ml-2 uppercase tracking-tight">💾 Memory</div>
...
<Tabs syncKey="experiment">
  ...
  <TabItem label="🔍 Inspect the Mind">
    ### Direct State Access
  ...
  <TabItem label="✍️ Manual Override">
...
In the **[next tutorial](/learn/agent-essentials/host-your-agent)**, we will conclude this journey by taking Jordan's assistant out of the terminal and **hosting it as a scalable service**.
