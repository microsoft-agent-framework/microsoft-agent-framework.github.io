...
time: "25 min"
samplePath: "ScrapeGraphAI.JobPageParser"
sourceUrl: "https://github.com/anktsrkr/scrapegraphai-csharp-sdk/tree/main/samples/ScrapeGraphAI.JobPageParser"
concepts:
  - Web extraction
  - Page scraping
  - Job parsing
...
prerequisites:
  ...
  - "An OpenAI-compatible chat endpoint such as LM Studio, Ollama, OpenAI, or Azure OpenAI-compatible routing"
  - "A tool-capable chat model loaded in your endpoint"
envVars:
  - "SGAI_API_KEY"
  - "OPENAI_BASE_URL"
  - "OPENAI_API_KEY"
...
expectedOutput:
  - "The app prints the selected ScrapeGraphAI tool: extract_from_page or scrape_page."
  - "The agent reads or extracts from the public job posting before answering."
  - "The agent extracts role facts such as title, location, skills, responsibilities, and hiring signals."
  - "The final answer includes a concise brief, source URL, and any missing or uncertain fields."
---
...
<div class="solid-callout solid-callout-info mb-8">
  ...
  <ul class="space-y-2.5 m-0 p-0 list-none text-sm text-indigo-900/80">
    ...
    <li class="flex items-center gap-2">🤖 **AI Provider**: An OpenAI-compatible endpoint such as LM Studio, Ollama, OpenAI, or a compatible hosted service.</li>
    <li class="flex items-center gap-2">🔑 **ScrapeGraphAI**: A valid `SGAI_API_KEY` for web scraping and extraction.</li>
    <li class="flex items-center gap-2">🌐 **Job URL**: A public job posting page you are allowed to access.</li>
  </ul>
</div>
...
Works with either approach. Use `ScrapePage` when you want a richer narrative brief; use `ExtractFromPage` when you want the prep plan grounded in specific extracted fields.
  ...
  <TabItem label="Recruiter intake">
```bash
...
Often better with `ScrapePage` because the agent can reason across the full posting text, but `ExtractFromPage` is useful when you want the gap analysis tied to explicit required and preferred skills.
  ...
  <TabItem label="Interview questions">
```bash
