...
prerequisites:
  ...
  - "Completion of 'State & Checkpoints'"
envVars:
  - "AZURE_OPENAI_ENDPOINT"
  - "AZURE_OPENAI_DEPLOYMENT_NAME"
runCommands:
  - "dotnet new func --name IncidentTriageWorkflow.Host --worker-runtime dotnet-isolated"
  ...
  - "func start"
expectedOutput:
  - "The Azure Functions host starts and lists a local Workflow endpoint."
  - "The workflow processes a cURL request and returns a JSON triage summary."
---
  ...
  <div class="flex flex-col md:flex-row gap-8 items-center">
    ...
    <div>
      ...
      <ul class="mt-4 space-y-2 text-xs text-indigo-900/60 list-none p-0">
        <li class="flex items-center gap-2">✅ **Accessibility**: Any internal system can now "ask" for triage via standard JSON/HTTP.</li>
        <li class="flex items-center gap-2">✅ **Scalability**: Azure Functions handle the heavy lifting of scaling the agent host.</li>
        <li class="flex items-center gap-2">✅ **Persistence**: Leveraging the Durable backend ensures that workflow progress and agent memory are automatically preserved.</li>
      </ul>
    </div>
  </div>
</div>
...
<div class="solid-callout solid-callout-info mb-8">
  ...
  <ul class="space-y-2.5 m-0 p-0 list-none text-sm text-indigo-900/80">
    <li class="flex items-center gap-2">🛠️ **Functions Core Tools**: Ensure `func` is available in your terminal.</li>
    <li class="flex items-center gap-2">📦 **Hosting Extension**: We use `Microsoft.Agents.AI.Hosting.AzureFunctions`.</li>
    <li class="flex items-center gap-2">💾 **Storage Backend**: A Durable Task Scheduler (DTS) must be running.</li>
  </ul>
</div>
...
<details class="premium-details my-8">
  ...
  <div class="premium-details-content">
    ...
    <div class="solid-callout solid-callout-info mb-6">
      ...
      <p class="text-[11px] text-indigo-900/60 mb-3">Add the following to your project's `host.json` to enable the Durable Task extension:</p>
...
```
    ...
    <Tabs syncKey="environment">
      <TabItem label="Local Emulator (Docker)">
      ...
      <TabItem label="Azure (Cloud)">
        <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">1. Provision Resource</p>
        <p class="text-[11px] text-slate-500 mb-4">Deploy a **Durable Task Scheduler** instance via the Azure Portal or Bicep templates.</p>
        <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">2. Assign Permissions</p>
        <p class="text-[11px] text-slate-500 mb-4">Enable **Managed Identity** on your Function App and assign it the <code class="text-[10px] bg-slate-100 px-1 rounded text-indigo-600 font-custom">Durable Task Data Contributor</code> role on the DTS resource.</p>
    ...
    <div class="solid-callout solid-callout-info mt-6">
      ...
      <p class="text-xs text-indigo-900/80 leading-relaxed">
        To scaffold and run your workflow host, you need the **Azure Functions Core Tools (v4.x)**. If you don't have them yet, [follow the official installation guide](https://learn.microsoft.com/en-us/azure/azure-functions/functions-run-local) for your platform.
      </p>
    </div>
  </div>
</details>
...
<Tabs syncKey="experiment">
  <TabItem label="⚡ Synchronous (Wait)">
    ### "Wait and See"
  ...
  <TabItem label="⏳ Asynchronous (Poll)">
    ### "Trigger and Poll"
