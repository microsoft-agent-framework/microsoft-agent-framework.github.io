...
prerequisites:
  ...
  - "Completion of 'Beyond Agents: Compound AI Systems'"
  - "Access to an AI Provider (for hybrid steps)"
envVars:
  ...
  - "OPENAI_MODEL_NAME"
runCommands:
  - "dotnet new console -n IncidentTriage.Workflow -f net10.0"
  - "cd IncidentTriage.Workflow"
  - "dotnet add package Microsoft.Agents.AI.Workflows"
  - "dotnet add package Microsoft.Agents.AI.OpenAI"
  - "dotnet restore"
  - "dotnet run"
expectedOutput:
  - "The workflow fetches telemetry, triggers the triage agent, and parses the agent response."
  - "The agent calls the priority assessment tool and returns PRIORITY: P0 for the checkout incident."
  - "The terminal uses color to separate workflow status, streamed agent output, and the final decision."
next: "/learn/advanced-orchestration/state-and-checkpoints"
---
  ...
  <div class="flex flex-col md:flex-row gap-8 items-center">
    ...
    <div>
      ...
      <p class="text-sm leading-relaxed text-indigo-900/70">
        In Jordan's world, reliability is non-negotiable. This workflow implements the **Hybrid AI Pattern**: a "Smart Brain" wrapped in a "Safe Shell." 
      </p>
      <ul class="mt-4 space-y-2 text-xs text-indigo-900/60 list-none p-0">
        <li class="flex items-center gap-2">✅ **Safety**: C# handles high-stakes data fetching and final routing logic.</li>
        <li class="flex items-center gap-2">✅ **Intelligence**: The AI handles the "messy" interpretation of telemetry data.</li>
        <li class="flex items-center gap-2">✅ **Protocol**: We use specialized "bridge" executors to translate between deterministic types and Agentic Chat Protocol.</li>
      </ul>
    </div>
  </div>
</div>
...
<div class="solid-callout solid-callout-info mb-8">
  ...
  <ul class="space-y-2.5 m-0 p-0 list-none text-sm text-indigo-900/80">
    ...
    <li class="flex items-center gap-2">🔄 **Workflows**: We will use the `Microsoft.Agents.AI.Workflows` package for orchestration.</li>
  </ul>
</div>
...
<div class="w-full bg-slate-50/50 rounded-[2rem] p-8 md:p-12 border border-slate-100/80 my-10 flex flex-col mermaid-wide transition-all hover:bg-slate-50">
...
The agent is also bound with `ForwardIncomingMessages = false`. This matters because an Agent executor can otherwise forward the incoming user message downstream before it sends the assistant response. Without this setting, `TriageSync` would see two separate `List<ChatMessage>` deliveries: first the forwarded user message, then the assistant response. For this workflow, the sync step should parse only Jordan's assistant report.
...
<div class="solid-callout solid-callout-info my-8">
  ...
  <p class="text-sm text-indigo-900/80 leading-relaxed m-0">
    The <code>[SendsMessage]</code> attribute isn't just for documentation. It enables **Protocol Validation**. By declaring that <code>TriageAdapter</code> sends <code>ChatMessage</code> and <code>TurnToken</code>, the <code>WorkflowBuilder</code> can verify that your graph is correctly connected before you even hit "Run."
  </p>
</div>
...
4.  **The Sync**: Since the Agent outputs a conversational list (`List<ChatMessage>`), the `TriageSync` executor extracts the core report so the final `EscalationEngine` can make a deterministic decision.
...
In the **[next tutorial](/learn/advanced-orchestration/state-and-checkpoints)**, we will ensure Jordan's triage assembly line is resilient to failures by implementing **Shared State** and **Checkpoints**.
